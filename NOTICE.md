...
- **No Pokémon asset file is committed to this repository or bundled into a
  release.** Search it; there are none.
...
- The one place Pokémon art appears in the repository is the README
  screenshots in `docs/img/`. Those are pictures **of the running software**,
  generated by `scripts/gen-shots.ts` — they depict the app rather than
  redistributing its assets, and no usable sprite can be extracted from them.
...
| README screenshots — `docs/img/*` | Generated by `scripts/gen-shots.ts` | Pictures of this app (see §2) |
